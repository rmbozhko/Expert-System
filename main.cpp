...
factValues		ft_evaluate_lpart(const Node const* node, std::vector<ExpSys::Tree*> treeStrg, std::map<std::string, Fact const*> factsStrg)
{	
...
factValues		ft_evaluate_rpart(const Node const* node, std::vector<ExpSys::Tree*> treeStrg, std::map<std::string, Fact const*> factsStrg)
{	
	if (node->GetType() == ExpSys::nodeType::Operation)
	{
		const Operation const*	oper = dynamic_cast<const ExpSys::Operation const*>(node);
...
int main(int argc, char const *argv[])
{
	...
	if (argc == 2)
	{
		if (strlen(argv[1]) > 0)
			...
			{
				try {
					if (!yyin && yyparse(treeStrg, factsStrg, factsOutput))
